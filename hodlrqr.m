function [YA, YB, YC, T, R, rk] = hodlrqr(A, Bl, Br, C, lvls, norm2, tol_or_rank)
    ...
    if lvls == 0
        ...
        S = hodlr_adj_apply(YA1, A.A12.Q)*A.A12.R + YB1.R'*hodlr_adj_apply(A.A22, YB1.Q)';
        Sp3 = YC1(1:size(Br,1), :)'*Br(:, n1+1:min(n1+n2,size(Br,2)));
        Sp4 = YC1(size(Br,1)+1:size(Br,1)+size(C,1), :)'*C(:, n1+1:min(n1+n2,size(C,2)));
        if ~isempty(Sp3)
            S = S + Sp3;
        end
        if ~isempty(Sp4)
        ...
        [QS, RS] = myqr(S, tol_or_rank*norm2);
        [QS, RS] = recompression(QS, RS, tol_or_rank*norm2);
        rk = max(rk, size(QS,2));
        QS = hodlr_adj_apply(T1, QS);
        [Q12, R12] = myqr(A.A12.Q*A.A12.R - hodlr_apply(YA1, QS)*RS, tol_or_rank*norm2);
        [Q12, R12] = recompression(Q12, R12, tol_or_rank*norm2);
        ...
        T12 = (hodlr_adj_apply(YA2, YB1.Q)*YB1.R)' + YC1(1:size(Br,1), :)'*YC2(1:size(Br2,1), :) + YC1(size(Br,1)+1:size(Br,1)+size(C,1), :)'*YC2(size(Br2,1)+1:size(Br2,1)+size(C2,1), :);
        [QT12, RT12] = myqr(T12, tol_or_rank);
        [QT12, RT12] = recompression(QT12, RT12, tol_or_rank);
